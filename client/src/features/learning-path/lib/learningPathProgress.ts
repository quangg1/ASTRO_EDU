...
/**
 * Giống normalizeCompletionKeys khi có id khớp curriculum tĩnh; nếu không khớp vẫn giữ key (bài chỉ có trên curriculum API).
 */
function normalizeLessonKeyedMapKeepingUnknown(map: LessonCompletionMap): LessonCompletionMap {
  const validIds = getAllLessonIds()
  const normalized: LessonCompletionMap = {}
  for (const rawKey of Object.keys(map)) {
    const key = String(rawKey || '').trim()
    if (!key || !map[rawKey]) continue
    const candidates = [key]
    try {
      candidates.push(decodeURIComponent(key))
    } catch {
    ...
    const canon =
      matched != null
        ? String(matched).trim()
        : candidates.map((c) => String(c || '').trim()).find((c) => c.length > 0) || key
    if (!canon) continue
...
export function loadLessonVisited3D(userId?: string | null): LessonVisited3DMap {
  ...
  try {
    ...
    const normalized = normalizeLessonKeyedMapKeepingUnknown(data) as LessonVisited3DMap
    const serialized = JSON.stringify(normalized)
...
function mergeVisited3DMaps(local: LessonVisited3DMap, server: LessonVisited3DMap): LessonVisited3DMap {
  return { ...server, ...local }
}
...
export async function syncLearningPathCompletion(userId?: string | null): Promise<LessonCompletionMap> {
  ...
  try {
    ...
    const serverIds = normalizeIdArray(
      Array.isArray(data?.data?.completedLessonIds) ? data.data.completedLessonIds : [],
    )
    const serverMasteredIds = normalizeIdArray(
      Array.isArray(data?.data?.masteredLessonIds) ? data.data.masteredLessonIds : [],
    )
    const visitedServerIds = normalizeIdArray(
      Array.isArray(data?.data?.visited3DLessonIds) ? data.data.visited3DLessonIds : [],
    )
    const serverLastLessonId = String(data?.data?.lastLessonId || '').trim() || null

    const mergedVisitedLocal = mergeVisited3DMaps(loadLessonVisited3D(userId), mapFromCompletedIds(visitedServerIds) as LessonVisited3DMap)
    saveLessonVisited3D(mergedVisitedLocal, userId)
    const mergedVisitedIds = completedIdsFromMap(mergedVisitedLocal)
    const serverVisitedSet = new Set(visitedServerIds)
    const needPushVisited = mergedVisitedIds.some((id) => !serverVisitedSet.has(id))

    const pushVisitedIfNeeded = async () => {
      if (needPushVisited) await pushVisited3DLessonIdsMerge(userId)
    }
