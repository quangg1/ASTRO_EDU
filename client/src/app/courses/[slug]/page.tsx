...
export default function CourseSlugPage() {
  ...
  const setRequestAgentOpen = useTutorContextStore((s) => s.setRequestAgentOpen)
  ...
  useEffect(() => {
    if (enrolledFromUrl === '1' && course && !course.enrollment) {
      fetchCourse(slug).then((c) => c && setCourse(c))
    }
  }, [enrolledFromUrl, course?.enrollment, slug])
  ...
  useEffect(() => {
    if (!course?.lessons || !lessonSlugFromUrl) return
  ...
  const progressBySlug = new Map(
    (course?.enrollment?.progress ?? []).map((p) => [p.lessonSlug, p.completed])
  )
  ...
  const currentIndex = selectedLesson
    ? lessons.findIndex((l) => l.slug === selectedLesson.slug)
    : -1
              ...
              <div className="flex-1 min-h-0 overflow-auto">
                {!isEnrolled && course.isPaid && (course.price ?? 0) > 0 ? (
                      ...
                      <div className="px-6 pb-6">
                        <button
                          ...
                          onClick={() => {
                            ...
                            router.replace(`/courses/${slug}?lesson=${encodeURIComponent(nextLesson.slug)}`, { scroll: false })
                          }}
                  ...
                  <div className="w-full h-full min-h-[400px] relative flex flex-col">
                    ...
                    {!['earth-history', 'solar-system', 'milky-way'].includes(
                    ...
                    {selectedLesson.quizQuestions && selectedLesson.quizQuestions.length > 0 ? (
                      <>
                        {selectedLesson.content && (
                          <p className="px-6 pt-6 text-sm text-gray-400 max-w-3xl">{selectedLesson.content}</p>
                        )}
                        <QuizLessonBlock
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (first) {
                      ...
                      router.replace(`/courses/${slug}?lesson=${encodeURIComponent(first.slug)}`, { scroll: false })
                    }
                  }}
