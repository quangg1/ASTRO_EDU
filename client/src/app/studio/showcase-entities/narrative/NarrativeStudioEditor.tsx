...
type Props = {
  entityId: string
  /** CMS showcase rows — cùng ngu?n merge texture v?i globe 3D. */
...
export function NarrativeStudioEditor({ entityId, showcaseContent }: Props) {
  ...
  const importLegacy = () => {
    ...
    setMessage('Ðã khôi ph?c d? li?u m?u — b?m Luu d? ghi DB.')
  }
  ...
  const onSave = async () => {
    ...
    if (!bundle.beats.length) {
      setMessage('Thêm ít nh?t m?t th?i k? tru?c khi luu.')
      return
    }
    setSaving(true)
    ...
    setSaving(false)
    setDataSource('db')
    setMessage('Ðã luu — Explore Deep History d?c t? planet-narratives.')
    notifyShowcaseCatalogChanged()
  }

  if (loading) {
    return <p className="text-xs text-ds-muted py-4">Ðang t?i Deep History…</p>
  }
  ...
  return (
    <div className="space-y-4 rounded-xl border border-violet-500/25 bg-gradient-to-b from-violet-950/30 to-black/20 p-4">
          ...
          <p className="text-xs text-ds-muted mt-1">
            ...
            {dataSource === 'db'
              ...
              : dataSource === 'legacy'
                ...
                : 'tr?ng (chua có th?i k?)'}
            {!entityHasExploreHistoryViewer(entityId) ? (
              <span className="text-amber-200/90"> · Explore chua có viewer 3D cho entity này</span>
            ) : null}
          </p>
        </div>
      ...
      <label className="block text-xs text-ds-muted">
        ...
        <span className="text-ds-subtle">(lesson id, cách nhau b?ng d?u ph?y — hi?n trên Explore Deep History)</span>
        <input
      ...
      <div className="flex gap-1 rounded-lg border border-ds-border p-1">
        {(
          [
            ...
            ['sites', 'Pin / d?a di?m'],
            ...(showFossilsTab ? ([['fossils', 'Hóa th?ch']] as const) : []),
          ] as const
        ).map(([id, label]) => (
          <button
            key={id}
      ...
      {subTab === 'beat' && !beat ? (
        <div className="rounded-lg border border-dashed border-violet-500/40 bg-black/20 p-8 text-center space-y-3">
          <p className="text-sm text-slate-300">Chua có th?i k? nào cho entity này.</p>
          <p className="text-xs text-ds-muted max-w-md mx-auto">
            Thêm th?i k? m?i — m?t s? entity có preset m?u khi chua luu DB.
          </p>
      ...
      {subTab === 'sites' ? (
        bundle.sites.length === 0 && !site ? (
          <div className="rounded-lg border border-dashed border-ds-border p-6 text-center space-y-2">
            <p className="text-xs text-ds-muted">Chua có pin — thêm d?a di?m trên b? m?t entity.</p>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(10rem,12rem)_1fr]">
            ...
            <div className="space-y-2">
              ...
              <label className="block space-y-1">
                <span className="text-[11px] text-slate-400">Lo?i d?a hình</span>
                <select
                  value={site.kind}
              ...
              {globeTextureUrl ? (
                ...
                <p className="text-[10px] text-amber-200/80 rounded-lg border border-amber-500/25 bg-amber-950/20 px-2 py-1.5">
                  Chua có texture equirectangular trong catalog — thêm <code className="text-amber-100">texturePath</code> cho entity
                  ho?c nh?p lat/lng tay.
                </p>
              )}
              <TextArea label="Blurb" value={site.blurbVi} onChange={(v) => patchSite(site.id, { blurbVi: v })} rows={2} />
              <StageMultiSelect
                label="Hi?n trong các th?i k?"
                ...
                placeholder="Ð? tr?ng = hi?n t?t c? th?i k? (ho?c quy t?c legacy)."
              />
              <label className="block space-y-1">
                ...
                >
                  ...
                  <option value="artistic">Minh h?a / tái d?ng</option>
                </select>
              </label>
              <ShowcaseMediaUrlField
                ...
                uploadContext={{
                  ...
                  entityId,
                  variant: `narrative/sites/${site.id}/cover`,
                }}
              />
            </div>
          </div>
      ...
      {subTab === 'fossils' ? (
        <p className="text-xs text-amber-100/90 rounded-lg border border-amber-500/30 bg-amber-950/20 p-3">
          Hóa th?ch Earth: hard-code + API /fossils — chua trong NarrativeBeat. Timeline ? tab «Th?i k? & panel».
        </p>
      ) : null}
      ...
      <footer className="flex flex-wrap gap-2 border-t border-ds-border pt-3">
        <button type="button" disabled={saving} onClick={() => void onSave()} className="rounded-lg bg-violet-600 px-4 py-2 text-sm text-white disabled:opacity-50">
          {saving ? 'Ðang luu…' : 'Luu Deep History'}
        </button>
