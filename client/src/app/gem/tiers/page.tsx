...
function TierColumn({
  ...
  return (
    ...
    >
      ...
      {isNext && !isCurrent && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-white/20 bg-[#1a1528] px-3 py-0.5 text-[10px] font-medium text-slate-300">
          Tiếp theo
        </span>
      )}
      <div className="text-center pt-2">
        ...
        <p className="mt-1 text-xs text-slate-400">{formatGemsEarnedRange(tier)}</p>
      ...
      <ul className="mt-5 flex-1 space-y-2.5 text-sm">
        {tier.perks.map((perk) => (
          <li key={perk.id} className="flex gap-2 text-slate-300">
            <Check
              className={`mt-0.5 h-4 w-4 shrink-0 ${perk.highlight ? 'text-cyan-400' : 'text-slate-500'}`}
              aria-hidden
            />
      ...
      <p className="mt-4 text-xs text-slate-500 leading-relaxed border-t border-white/5 pt-4">
        {tier.taglineVi}
      </p>
    </article>
  )
...
function ProgressBanner({ progress, gemBalance }: { progress: LearnerTierProgress; gemBalance: number }) {
  ...
  return (
    <Card className="p-5 sm:p-6 border-cyan-500/25 bg-[#101224]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-wider text-slate-500">Hạng hiện tại</p>
  ...
  return (
    <div className="space-y-8 max-w-6xl pb-12">
      <header>
        <Link
          href="/gem"
          className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-cyan-200 mb-4"
        >
      ...
      {loading && (
        <p className="text-sm text-slate-500 py-12 text-center">Đang tải hạng…</p>
      )}
              ...
              <table className="w-full min-w-[640px] text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-white/[0.03]">
                    <th className="text-left p-4 text-slate-400 font-medium w-[40%]">Quyền lợi</th>
                    {sortedTiers.map((t) => (
                      <th
                        key={t.id}
                        className={`p-4 text-center font-medium ${
                          t.id === currentId ? 'text-cyan-200' : 'text-slate-300'
                        }`}
                      >
      ...
      <section className="rounded-2xl border border-white/10 bg-[#0c0a12] p-5 text-sm text-slate-400 space-y-2">
        <p>
