/**
 * Crawl tin thiên văn từ RSS — dùng chung cho script CLI và scheduler API.
 */
...
const { ensureNewsForumRecord } = require('./forumBootstrapService');
...
async function runNewsCrawl(options = {}) {
  const fetchFull =
    options.fetchFull ??
    String(process.env.CRAWL_FETCH_FULL || 'false').toLowerCase() === 'true';
  const fetchOgIfMissing =
    options.fetchOgIfMissing ??
    String(process.env.CRAWL_FETCH_OG_IMAGE || 'true').toLowerCase() !== 'false';
  const maxItems = Math.min(
    200,
    Math.max(1, parseInt(options.maxItems ?? process.env.CRAWL_MAX_ITEMS ?? '25', 10) || 25),
  );
  const delayMs = Math.max(
    0,
    parseInt(options.delayMs ?? process.env.CRAWL_DELAY_MS ?? '900', 10) || 0,
  );

  const feedUrls = options.feedUrls
    ? options.feedUrls
    : process.env.CRAWL_RSS_FEEDS
      ? process.env.CRAWL_RSS_FEEDS.split(',')
          .map((s) => s.trim())
          .filter(Boolean)
      : DEFAULT_FEEDS;

  const log = options.logger?.info ? (msg) => options.logger.info(msg) : (msg) => console.log(msg);

  const forum = await ensureNewsForumRecord();
          ...
          : contentSource === 'jsonld'
            ...
            : contentSource === 'wp-main'
              ...
              : contentSource === 'readability'
                ? '[read]'
                : '[rss]';
        log(`${tag} ${title.slice(0, 72)}${title.length > 72 ? '…' : ''}`);
      } catch (e) {
        if (e.code === 11000) {
          skipped += 1;
        } else {
  ...
  const summary = { created, skipped, errors, ranAt: new Date().toISOString() };
  log(`News crawl xong: tạo mới=${created}, bỏ qua/trùng=${skipped}, lỗi=${errors}`);
  return summary;
}
