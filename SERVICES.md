...
| **ai** | 5005 | AI service (Python) – RAG, security, multimodal (text + ảnh), gọi LM Studio |
...
# (Tùy chọn) Terminal 9 – AI service (Python: RAG, security, multimodal). Cần embedding (5004) + LM Studio
...
### 5. Seed forums + crawl tin thiên văn
```bash
cd services/community && npm run seed && npm run crawl-news
```
